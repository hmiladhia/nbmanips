...
class Cell:
    _cell_types: ClassVar[dict[str, type[Cell]]] = {}
    ...
    def set_source(self, content: str | list[str], text=False):
        if text:
            if isinstance(content, str):
                raise ValueError("content is not of type str if text=True")
    ...
    def byte_size(self, output_types: set | None = None, ignore_source=False):
        """
...
class MarkdownCell(Cell, cell_type="markdown"):
    ...
    def attach(self, path: str | Path, attachment_name: str | None = None):
        import base64
