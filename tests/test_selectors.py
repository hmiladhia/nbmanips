...
def test_selector_args(nb1: Notebook, selector, args, expected):
...
def test_list_selector_args(nb1: Notebook, selector, args, expected):
    assert nb1.select(selector, *args).first() == expected


def test_list_or_selector(nb1: Notebook):
...
def test_slice_selector(nb1: Notebook, slice_: list):
    assert nb1.select(slice(*slice_)).list() == list(range(*slice_))


def test_negative_slice_selector(nb1: Notebook):
...
def test_none_selector(nb1: Notebook):
    assert nb1.select(None).list() == [i for i in range(len(nb1))]


def test_selector_selector(nb1: Notebook):
...
def test_has_output_type1(nb1: Notebook, output_type, expected):
    assert nb1.select("has_output_type", output_type).list() == expected


def test_is_empty(nb3: Notebook):
    assert nb3.select("is_empty").list() == [5, 8]


def test_has_byte_size(nb3: Notebook):
...
def test_has_html_tag(nb6: Notebook):
...
def test_has_tag(nb6_0: Notebook):
